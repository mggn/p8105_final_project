...
flu_adult_df %>% 
  ...
  ggplot(aes( x = flu_shot, fill = sex)) + geom_bar(position = "dodge")  + scale_fill_manual(values = wes_palette("GrandBudapest2"))
```
...
flu_adult_df %>% 
 ...
 filter(age > 65) %>%
  filter(bmi != "unknown") %>% 
  ggplot(aes(x = bmi, fill = flu_shot )) + geom_bar(position = "dodge")  + scale_fill_manual(values = wes_palette("GrandBudapest2")) 
...
flu_adult_df %>% 
  ...
  ggplot(aes(y = coverage, x = flu_shot_month, color = coverage, alpha = .04)) + geom_jitter() + scale_fill_manual(values = wes_palette("GrandBudapest2")) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) 
```
...
flu_adult_df %>%
  ...
  filter(flu_shot != "not applicable") %>% 
  mutate(coverage = fct_infreq(coverage)) %>% 
  ggplot(aes(x = coverage, fill = flu_shot)) + geom_bar(position = "dodge") + scale_fill_manual(values = wes_palette("GrandBudapest2"))
```
...
flu_adult_df %>% 
  ...
  ggplot(aes(x = flushot_preg, fill = flushot_preg, alpha = .5)) + geom_density()  + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + scale_fill_manual(values = wes_palette("GrandBudapest2"))
```
...
flu_adult_df %>% 
  ...
  ggplot(aes(x = emergency_visits,  fill = flu_shot)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
flu_adult_df %>% 
  ...
  ggplot(aes(x = urgentcare_visits,  fill = flu_shot)) + geom_bar(position = "dodge") + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
```{r}
flu_adult_df %>% 
  filter(health != "no response") %>% 
  filter(health != "Don't know") %>% 
   filter(flu_shot != "no response") %>% 
  filter(flu_shot != "not applicable") %>% 
  filter(flu_shot != "don't know") %>% 
  ggplot(aes(x = health, fill = flu_shot)) + geom_density(alpha = .4, adjust = .5) + facet_grid(nh_race ~ .)
```
...
flu_adult_df %>% 
  ...
  ggplot(aes(x = errands_alone_diff, fill = flu_shot)) + geom_bar(position = "dodge")  + scale_fill_manual(values = wes_palette("BottleRocket1")) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
```
...
flu_adult_df %>% 
  ...
  filter(errands_alone_diff != "no response") %>% 
  filter(errands_alone_diff != "don't know") %>%
  ggplot(aes(x = errands_alone_diff, fill = flu_shot)) + geom_bar(position = "dodge")  + scale_fill_manual(values = wes_palette("BottleRocket1")) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + transition_states(
    flu_shot,transition_length = 2, state_length = 5) + enter_grow() + exit_shrink() + ease_aes('sine-in-out')
```
...
flu_adult_df %>% 
  ...
  filter(flu_shot != "don't know") %>% 
  filter(sex != "No response") %>% 
  ggplot(aes( x = sex, fill = flu_shot)) + geom_bar(position = "dodge")  + scale_fill_manual(values = wes_palette("GrandBudapest2")) + transition_states(
   flu_shot,transition_length = 2, state_length = 5) + enter_grow() + exit_shrink() + ease_aes('sine-in-out')
```
...
flu_adult_df %>% 
  ...
  filter(coverage != "don't know") %>%
  filter(coverage != "NA") %>% 
  ggplot(aes(x = coverage, y = flu_shot_month, color = coverage, alpha = .04)) + geom_jitter() + scale_fill_manual(values = wes_palette("GrandBudapest2")) + transition_states(
   coverage,transition_length = 2, state_length = 5) + enter_grow() + exit_shrink() + ease_aes('sine-in-out')
```

hex sticker?
**problem is loading the image into the vector**
```{r}
imgurl <- system.file("hexstick/tina2.png", package="hexSticker")
```
trying hex sticker

*this isn't where the problem lies*
```{r}
sticker(tina, package="hexSticker", p_size=20, s_x=1, s_y=.75, s_width=.6,
        filename="./hexstick/trial3.png")
```
