...
plot_prototype =
  ...
  filter(state == c("Alabama":"Connecticut"))%>%
  drop_na(flushot_yn, state) %>%
  select (state, flushot_yn) %>%
  group_by(state, flushot_yn)%>%
  summarize(n_obs = n()) %>%
  ggplot(aes(fill = flushot_yn, y = n_obs, x = flushot_yn))+
  ...
  #getting somewhere. For the plot above, think about splitting up states. Also percentages are probably more meaningful
  #dodge plot for readability
  #another plot: bar with response on x, count on y, facet_wrap by state
  #marriage of qualitative and quantitative
  #snapshot of right now
  #when was flu vacc first available?
